// vendored from @imgly/background-removal-node 1.4.5 with local dependency cleanup
...
var SUPPORTED_MODELS = /* @__PURE__ */ new Set(["small", "medium", "large"]);
var SUPPORTED_OUTPUT_FORMATS = /* @__PURE__ */ new Set([
  "image/png",
  "image/jpeg",
  "image/webp",
  "image/x-rgba8",
...
var DEFAULT_PUBLIC_PATH = `${pathToFileURL(
  path.resolve(path.dirname(fileURLToPath(import.meta.url)))
).toString()}/`;
function validateConfig(configuration) {
  const source = configuration ?? {};
  const config = {
    publicPath: source.publicPath ?? DEFAULT_PUBLIC_PATH,
    debug: source.debug ?? false,
    proxyToWorker: source.proxyToWorker ?? true,
    fetchArgs: source.fetchArgs ?? {},
    progress: source.progress,
    model: source.model ?? "medium",
    output: {
      format: source.output?.format ?? "image/png",
      quality: source.output?.quality ?? 0.8
    }
  };
  config.publicPath = config.publicPath.replace("${PACKAGE_NAME}", package_default.name).replace("${PACKAGE_VERSION}", package_default.version);
  if (!isURI(config.publicPath)) {
    throw new Error("String must be a valid uri");
  }
  if (typeof config.debug !== "boolean") {
    throw new Error("debug must be a boolean");
  }
  if (typeof config.proxyToWorker !== "boolean") {
    throw new Error("proxyToWorker must be a boolean");
  }
  if (typeof config.fetchArgs !== "object" || config.fetchArgs === null) {
    throw new Error("fetchArgs must be an object");
  }
  ...
  if (!SUPPORTED_MODELS.has(config.model)) {
    throw new Error("model must be one of: small, medium, large");
  }
  if (!SUPPORTED_OUTPUT_FORMATS.has(config.output.format)) {
    throw new Error("output.format is invalid");
  }
  if (typeof config.output.quality !== "number" || Number.isNaN(config.output.quality)) {
    throw new Error("output.quality must be a number");
  }
  if (config.debug)
...
var init = async (configuration) => {
  const cacheKey = JSON.stringify(configuration ?? {});
  if (!inferenceCache.has(cacheKey)) {
    inferenceCache.set(cacheKey, initInference(configuration));
  }
